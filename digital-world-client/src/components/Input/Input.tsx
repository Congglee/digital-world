  ...
  return (
    <div className={twMerge('relative', className)}>
      ...
      {rest.type === 'password' && openEye && (
        <Eye
          className={twMerge('w-5 h-5 absolute top-[8px] right-[5px] cursor-pointer', classNameEye)}
          onClick={toggleEye}
        />
      )}
      {rest.type === 'password' && !openEye && (
      ...
      <div className={twMerge('mt-0.5 text-red-600 min-h-[1.25rem] text-sm', classNameError)}>{errorMessage}</div>
    </div>
  )
}
